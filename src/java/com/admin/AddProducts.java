...
public class AddProducts extends HttpServlet {
	private final String UPLOAD_DIRECTORY = "C:/Users/azads/Documents/NetBeansProjects/BMS/web/uploads";
	@Override
			...
			try {
				...
				for (FileItem item : multiparts) {
					if (!item.isFormField()) {
						...
						FileItem quantity = (FileItem) multiparts.get(4);
						q_quantity = Integer.parseInt(quantity.getString());
						
						FileItem fstatus = (FileItem) multiparts.get(5);
						status = fstatus.getString();
						
						FileItem pcategory = (FileItem) multiparts.get(6);
				...
				try {
					...
					int i = DatabaseConnection.insertUpdateFromSqlQuery("insert into tblproduct(id,active,code,description,image,image_name,name,price,mrp_price,product_category, quantity) values('" + id+ "','" + status + "','"+code+"','" + descrip + "','" + imagePath + "','"+imageName+"','"+productName+"','"+productPrice+"','"+mrpPrice+"','"+category+"','"+q_quantity+"')");
					if (i > 0) {
